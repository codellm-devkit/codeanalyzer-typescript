...
| `entrypoints` | `Record<string, TSEntrypoint[]>` | Framework name → detected roots. Empty at level 1. |
...
| `content_hash`, `last_modified`, `file_size` | `string` / `number` / `number` | Cache-invalidation metadata (nullable). |
...
| `code` | `string \| null` | The source body. |
...
- **`TSImport`** — `module` specifier, `name`, `alias`, `is_type_only`, `import_kind` (`named` \| `default` \| `namespace` \| `side_effect`), and span.
...
- **`TSDecorator`** — `name`, checker-resolved `qualified_name`, raw `positional_arguments` and `keyword_arguments` (source fragments for finders to parse), and span.
