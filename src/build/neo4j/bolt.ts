...
import { TS_MARKER, applicationPrefix, chunk, descendantPrefix } from "./rows";
 ...
 */
/**
 * `--eager` purge (#140): everything under this application's prefix, anchored on `TSCanNode` so
 * the prefix predicate seeks an index. ONE statement covers both language namespaces now that the
 * application is the outermost segment. `$prefix` is the `/`-terminated descendant prefix from
 * `applicationPrefix`, never a bare app id — a bare id also matches `can://appXtra/...`.
 */
export const EAGER_PURGE =
  `MATCH (n:${TS_MARKER}) WHERE n.id STARTS WITH $prefix CALL { WITH n DETACH DELETE n } IN TRANSACTIONS OF 5000 ROWS`;
/** The per-module purge (#140): the module by equality, its subtree by `/`-prefix; `$keys` survive. */
const PURGE_MODULE_EDGES =
  `MATCH (x:${TS_MARKER}) WHERE x.id = $mid OR x.id STARTS WITH $pre MATCH (x)-[r]->() DELETE r`;
const PURGE_MODULE_STALE =
  `MATCH (x:${TS_MARKER}) WHERE (x.id = $mid OR x.id STARTS WITH $pre) AND NOT x.id IN $keys DETACH DELETE x`;
/** The orphan prune (#140): modules inside this app's prefix that the run no longer emits, with their subtrees. */
const PRUNE_VANISHED =
  `MATCH (m:TSModule:${TS_MARKER}) WHERE m.id STARTS WITH $prefix AND NOT m.id IN $present ` +
  `CALL { WITH m MATCH (x:${TS_MARKER}) WHERE x.id = m.id OR x.id STARTS WITH m.id + '/' DETACH DELETE x } ` +
...
export async function boltWriter(
  ...
  try {
    ...
    // prefix, and applicationPrefix refuses an empty one (#140). project() always emits the
    // Application row, so this only trips on a hand-built GraphRows.
    const prefix = applicationPrefix(appId);
    ...
    if (eager) {
      await withSession(session, async (s) => {
        await s.run(EAGER_PURGE, { prefix });
      });
      log.info(`neo4j(bolt): --eager, purged the existing graph under ${prefix}`);
    }

    // 3. diff content_hash.
    const dbHash = new Map<string, string | null>();
    await withSession(session, async (s) => {
      // Keyed by module ID inside this application's prefix (#140): a file key alone collides
      // across applications; the id carries application, language and file.
      const res = await s.run(
        "MATCH (m:TSModule) WHERE m.id STARTS WITH $prefix RETURN m.id AS k, m.content_hash AS h",
        { prefix },
      );
    ...
    for (const [m, nodes] of byModule) {
      ...
      if (!mid || !mid.startsWith(prefix)) {
        throw new Error(`neo4j: module ${m} has no can:// id under ${prefix}; refusing to scope a purge on it`);
      }
    ...
    for (const m of changed) {
      ...
      if (eager) {
        // The module by equality, its subtree by `/`-prefix (#140). Application-scoped by
        // construction: the id carries the app, and one marker spans both language namespaces.
        const mid = moduleIdOf.get(m)!;
        const params = { mid, pre: descendantPrefix(mid), keys };
        await withSession(session, async (s) => {
          await s.executeWrite(async (tx: any) => {
            await tx.run(PURGE_MODULE_EDGES, params);
            await tx.run(PURGE_MODULE_STALE, params);
          });
        });
      }
    ...
    if (fullRun && eager) {
      // Scoped on this app's prefix (#140); a second TypeScript app in the same database, whose
      // modules are all "not in $present", is outside the prefix.
      const present = [...moduleIdOf.values()];
      let pruned = 0;
      await withSession(session, async (s) => {
        const res = await s.run(PRUNE_VANISHED, { present, prefix });
